...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast(error?.reponse?.data?.message || error.message)
    }
  ...
  return (
    <div className="flex items-center justify-center min-h-screen">
      <form onSubmit={handleSubmit} className="sm:w-[350px] w-full text-center border border-gray-300/60 rounded-2xl px-8 bg-white">
        ...
        <p onClick={() => setState(prev => prev === "login" ? "signUp" : "login")} className="text-gray-500 text-sm mt-3 mb-11">{state === "login" ? "Don't have an account?" : "Already have an account?"} <a href="#" className="text-green-500 hover:underline">click here</a></p>
      </form>
    </div>
  )
}
